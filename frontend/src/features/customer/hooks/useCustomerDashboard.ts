...
export function useCustomerDashboard() {
    ...
    const [stats, setStats] = useState<DashboardStats>({ totalTrips: 0, completedTrips: 0, totalSpent: 0, totalDistance: 0 });
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            ...
            const totalSpent = completedTrips.reduce((acc, t) => acc + (Number(t.actual_fare) || Number(t.estimated_fare) || 0), 0);
            const totalDistance = completedTrips.reduce((acc, t) => acc + (Number(t.distance_km) || 0), 0);
            setStats({ totalTrips: allTrips.length, completedTrips: completedTrips.length, totalSpent, totalDistance });

            const recentCompleted = completedTrips.slice(0, 3);
            const formattedTrips: RecentTripDisplay[] = recentCompleted.map(trip => ({
                id: trip.id, pickup: trip.pickup_address, destination: trip.destination_address,
                date: trip.created_at, fare: Number(trip.actual_fare) || Number(trip.estimated_fare) || 0, status: trip.status,
                driverName: trip.driver ? `${trip.driver.first_name} ${trip.driver.last_name}` : 'Driver',
                driverRating: trip.driver?.rating || 4.5, cabType: trip.cab?.cab_type || 'Sedan', distance: `${trip.distance_km} km`,
            }));
        ...
        try {
            ...
            const formattedAddresses: SavedAddressDisplay[] = addresses.map(addr => ({
                id: addr.id, label: addr.label, address: addr.address, icon: addr.icon || '📍', color: 'bg-gray-100',
            }));
    ...
    return {
        hoveredAction, recentTrips, savedAddresses, stats, isLoading,
        setHoveredAction, getGreeting,
    };
}
