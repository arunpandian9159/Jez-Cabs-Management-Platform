  ...
  const fetchTrips = useCallback(async () => {
    try {
      ...
      const formatted: TripDisplay[] = data.map((trip) => {
        const actualFare = trip.actual_fare
          ? parseFloat(String(trip.actual_fare))
          : 0;
        const estimatedFare = trip.estimated_fare
          ? parseFloat(String(trip.estimated_fare))
          : 0;
