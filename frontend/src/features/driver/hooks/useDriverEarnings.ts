...
export function useDriverEarnings() {
  ...
  const [earningsSummary, setEarningsSummary] =
    useState<EarningsSummaryDisplay>({
      today: 0,
      week: 0,
      month: 0,
      pendingPayout: 0,
  ...
  const fetchEarningsData = useCallback(async () => {
    try {
      ...
      const formattedTxns: TransactionDisplay[] = (
      ...
      const formattedBreakdown: DayBreakdownDisplay[] = (
  ...
  const avgPerTrip =
    totalWeeklyTrips > 0 ? earningsSummary.week / totalWeeklyTrips : 0;
  const avgPerDay = earningsSummary.week / 7;
  const bestDay =
    weeklyBreakdown.length > 0
      ? weeklyBreakdown.reduce((max, day) =>
