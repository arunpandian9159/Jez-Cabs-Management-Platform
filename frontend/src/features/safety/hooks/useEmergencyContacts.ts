...
const initialNewContact = { name: '', phone: '', relationship: '', notifyOnRide: false };
...
export function useEmergencyContacts() {
    ...
    const handleToggleNotify = async (id: string) => {
        ...
        if (!contact) return;
        try {
            await safetyService.updateEmergencyContact(id, { notify_on_trip_start: !contact.notifyOnRide });
            setContacts(contacts.map(c => c.id === id ? { ...c, notifyOnRide: !c.notifyOnRide } : c));
        } catch (error) {
