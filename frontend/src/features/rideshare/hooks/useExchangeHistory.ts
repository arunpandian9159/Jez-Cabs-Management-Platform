...
import {
  rideshareService,
  ExchangeHistoryItem as APIExchangeHistoryItem,
} from '@/services';
...
export function useExchangeHistory() {
  ...
  const fetchExchangeHistory = useCallback(async () => {
    try {
      ...
      const formatted: ExchangeHistoryDisplay[] = history.map(
        (item: APIExchangeHistoryItem) => ({
          id: item.id,
          type: item.type,
          status: item.status,
          trip: {
          ...
          host: item.host
            ? { name: item.host.name, rating: item.host.rating }
            : undefined,
  ...
  return {
    ...
    bookedCount,
    hostedCount,
    totalCount: exchangeHistory.length,
    setActiveTab,
  };
}
