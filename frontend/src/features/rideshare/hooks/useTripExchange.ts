...
export function useTripExchange() {
    ...
    const fetchCommunityTrips = useCallback(async () => {
        try {
            ...
            const formatted: CommunityTripDisplay[] = trips.map((trip: APICommunityTrip) => ({
                id: trip.id,
                type: trip.type,
                poster: { name: trip.poster.name, rating: trip.poster.rating, trips: trip.poster.trips },
                from: trip.from,
                to: trip.to,
                date: trip.date,
                time: trip.time,
    ...
    const avgPricePerSeat = communityTrips.length > 0
        ? Math.round(communityTrips.reduce((sum, t) => sum + t.pricePerSeat, 0) / communityTrips.length)
        : 0;

    return {
        activeTab, searchQuery, showFilters, selectedTrip, communityTrips, filteredTrips, isLoading, error,
        totalSeats, avgPricePerSeat, totalTrips: communityTrips.length,
        setActiveTab, setSearchQuery, setShowFilters: () => setShowFilters(!showFilters), setSelectedTrip,
    };
}
