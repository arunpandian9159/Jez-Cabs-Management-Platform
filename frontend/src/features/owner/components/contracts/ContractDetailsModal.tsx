...
export function ContractDetailsModal({
    ...
    const getContractIcon = (type: string) => {
        switch (type) {
            case 'driver': return <User className="w-8 h-8 text-primary-600" />;
            case 'platform': return <FileText className="w-8 h-8 text-accent-600" />;
            default: return <Car className="w-8 h-8 text-success-600" />;
        }
    };
            ...
            {contract && (
                <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-4">
                        <div className={`w-16 h-16 rounded-lg flex items-center justify-center ${getIconBgClass(contract.type)}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p className="font-medium text-gray-900">{contract.vehicleAssigned}</p>
                        </div>
                        {contract.commission && (
                            <div>
                                <p className="text-sm text-gray-500">Commission Rate</p>
                                <p className="font-medium text-gray-900">{contract.commission}%</p>
                            </div>
                        )}
                        {(contract.monthlyTarget ?? 0) > 0 && (
                            <div>
                                <p className="text-sm text-gray-500">Monthly Target</p>
                                <p className="font-medium text-gray-900">{formatCurrency(contract.monthlyTarget ?? 0)}</p>
                            </div>
                        )}
                    </div>
