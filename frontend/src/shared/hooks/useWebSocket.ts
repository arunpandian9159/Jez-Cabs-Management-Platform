 ...
 */
export function useDriverLocationSocket(tripId: string | undefined, enabled = true) {
...
export interface TripStatusMessage {
    type: 'driver_assigned' | 'driver_en_route' | 'driver_arrived' | 'trip_started' | 'trip_completed' | 'trip_cancelled';
    driver?: {
        id: string;
        name: string;
        photo: string | null;
 ...
 */
export function useTripStatusSocket(tripId: string | undefined, enabled = true) {
    const [tripStatus, setTripStatus] = useState<TripStatusMessage | null>(null);
    const [driver, setDriver] = useState<TripStatusMessage['driver'] | null>(null);
