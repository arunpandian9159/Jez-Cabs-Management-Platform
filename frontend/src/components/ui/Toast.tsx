...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Action
        ref={ref}
        className={cn(
            'inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-gray-200 bg-transparent px-3 text-sm font-medium ring-offset-white transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-700 dark:ring-offset-gray-900 dark:hover:bg-gray-800 dark:focus:ring-gray-600',
            className
        )}
        {...props}
    />
...
const ToastIcon = ({ variant }: { variant?: 'success' | 'error' | 'warning' | 'info' | 'default' }) => {
    ...
    switch (variant) {
        case 'success':
            return <CheckCircle className={cn(iconClasses, 'text-success-600')} />;
        case 'error':
            return <AlertCircle className={cn(iconClasses, 'text-error-600')} />;
        case 'warning':
            return <AlertTriangle className={cn(iconClasses, 'text-warning-600')} />;
        case 'info':
            return <Info className={cn(iconClasses, 'text-info-600')} />;
        default:
            return null;
    }
};
