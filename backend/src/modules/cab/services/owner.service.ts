  ...
  async getDriver(driverId: string, ownerId: string) {
    ...
    if (!cab || !cab.assigned_driver) {
      throw new NotFoundException(
        'Driver not found or not assigned to your cabs',
      );
    }
    ...
    for (const cab of cabs) {
      ...
      if (cab.insurance_expiry) {
        ...
        contracts.push({
          ...
          vehicleAssigned: `${cab.make} ${cab.model} (${cab.registration_number})`,
          startDate: cab.created_at.toISOString(),
          endDate: cab.insurance_expiry.toISOString
            ? cab.insurance_expiry.toISOString()
            : new Date(cab.insurance_expiry).toISOString(),
          status,
          premium: 15000,
          documents: [] as string[],
        });
